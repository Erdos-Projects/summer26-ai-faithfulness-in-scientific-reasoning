...
def make_cover_fig(n_models=39, n_train=800):
    ...
    y = _block(fig, y, "What you can conclude", [
        ...
        "• Which demands drive failure? The tallest right-panel bars name the dimensions whose high demand "
        "most raises that model's failure odds. On CharXiv, figure-grounding dims GS (Gestalt/Shape) and "
        "MA (Multi-Element Aggregation) dominate, followed by MCu (metacognition), VL and AS.",
        "• Do models share a failure signature? Compare the right panel across pages — here the same few "
        "dims recur, so failure is driven by shared visual-grounding demand, not model-specific quirks.",
    ...
    y = _block(fig, y, "Caveats", [
        ...
        "• Permutation importance only (drop-column omitted for tractability across 40 models); "
        "inter-correlated dims can share credit.",
        "• Importance shows MAGNITUDE, not direction: a dim can be informative here while, in the "
        "Mann-Whitney charts, higher demand went with CORRECT answers (e.g. GS). The RF flags it either way.",
    ...
    fig.text(0.045, 0.022, "Outputs: charxiv_analysis/random_forest/ — this PDF, 40 PNGs, "
             "rf_cv_results.csv, rf_importance_long.csv.", fontsize=8, color="0.45", va="top")
    return fig
...
def run_one(model, X, y, groups, dims, names, pdf, cv_rows, imp_rows, label_out):
    auc, bacc, brier, perms, nfold = evaluate(X, y, groups, dims)
    base_rate = y.mean()
...
def main():
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        run_one("COMBINATION_ALL", np.array(Xp), np.array(yp), np.array(gp),
                dims, names, pdf, cv_rows, imp_rows, "combo")
        ...
        for m in models:
            ...
            if minority < MIN_MINORITY:
                ...
                continue
            run_one(m, X, y, g, dims, names, pdf, cv_rows, imp_rows, "model")
