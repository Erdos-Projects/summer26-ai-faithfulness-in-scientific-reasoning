export const meta = {
  name: 'sciclaimeval-rubric-dispatch',
  description: 'Self-fetching SciClaimEval DeLeAn demand-rubric pass: stage 1 refreshes the resume-aware manifest and returns the next N missing cells; stage 2 fans them out to isolated Sonnet subagents. Scores are scraped into annotations_sciclaimeval.db by collect_cli afterward.',
  phases: [
...
const fetchCmd =
  `cd ${REPO} && ${py} -m sciclaimeval_scoring.prepare --pass 1 --dims ${DIMS} >/dev/null 2>&1 && ` +
  `${py} -c "import json,os; ` +
  `m=json.load(open('sciclaimeval_scoring/prompts/pass_01/pass_01_manifest.json')); ` +
  `tags=list(m.keys())[:${batch}]; ` +
  `print(json.dumps({'promptDir':os.path.abspath('sciclaimeval_scoring/prompts/pass_01'),'tags':tags}))"`
...
await parallel(tags.map((tag) => () => {
  ...
  return agent(
    ...
    `(N = an integer 0-5). (5) STOP. Use ONLY the Read tool (twice). Do NOT verify the claim.`,
