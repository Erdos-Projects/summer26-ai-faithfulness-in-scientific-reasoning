...
from rubric_scoring import config, db, items as items_mod
from rubric_scoring.prompts import build_prompt, tag, DIMS

SKILL_VERSION = "0.1"
MODEL = "claude-sonnet-4-6"
DEVIATIONS = ("temp!=0 (subagent path; measured ~82% modal/89% within-1); "
              "output cap instructed not enforced; reasoning off; captions omitted; "
              "DeLeAn v1.0 textual-only anchors on multimodal items")
...
def prepare(pass_no, dims, out_dir, limit=None):
    ...
    db.register_pass(conn, pass_no, skill_version=SKILL_VERSION, model=MODEL,
                     active_dims=",".join(dims), item_source="SciVer charts (val+test)",
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    out = a.out or str(config.repo_root() / "rubric_scoring" / "prompts" / f"pass_{a.pass_no:02d}")
