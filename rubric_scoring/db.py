...
def register_pass(conn, pass_no, *, skill_version, model, active_dims, item_source,
    ...
    conn.execute("INSERT OR REPLACE INTO pass VALUES(?,?,?,?,?,?,?,?,?)",
                 (pass_no, created if created is not None else _now(), skill_version, model, active_dims, item_source,
                  n_items, params, deviations))
    conn.commit()
...
def upsert_annotation(conn, row: dict):
    ...
    upd = (
        ",".join(f"{c}=excluded.{c}" for c in _FIELDS if c not in ("pass", "item_id", "dim_code"))
