...
from charxiv_scoring import config, db
from charxiv_scoring.scrape import collect
...
if __name__ == "__main__":
    ...
    ap.add_argument("--since", type=int, default=None,
                    help="only scrape transcripts modified within the last N seconds "
                         "(incremental; safe to set >= time since the previous collect)")
    a = ap.parse_args()
    conn = db.connect(config.db_path()); db.init(conn)
    with open(a.manifest) as f:
        manifest = json.load(f)
    session_id = os.environ.get("CLAUDE_CODE_SESSION_ID", "unknown")
    n = collect(conn, a.pass_no, manifest, session_id=session_id, since_seconds=a.since)
