...
from charxiv_scoring import config, db, items as items_mod
from charxiv_scoring.prompts import build_prompt, tag, DIMS
...
DEVIATIONS = ("temp!=0 (subagent path; SciVer-measured ~82% modal/89% within-1); "
              ...
              "DeLeAn v1.0 textual-only anchors on multimodal chart-QA items; "
              "task instance = answer reasoning question (vs claim verification in SciVer/SciClaimEval)")
...
def prepare(pass_no, dims, out_dir, limit=None):
    ...
    db.register_pass(conn, pass_no, skill_version=SKILL_VERSION, model=MODEL,
                     active_dims=",".join(dims), item_source="CharXiv val reasoning (1000)",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dims", default="AS,QLq,QLl,MCr,AT,VO,VL,GS,MA,MCu,CL,KNf")
    ap.add_argument("--out", default=None)
    ap.add_argument("--limit", type=int, default=None)
    a = ap.parse_args()
    out = a.out or str(config.repo_root() / "charxiv_scoring" / "prompts" / f"pass_{a.pass_no:02d}")
