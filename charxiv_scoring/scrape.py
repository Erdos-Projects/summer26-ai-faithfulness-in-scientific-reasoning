...
from charxiv_scoring import db, config
from charxiv_scoring.parse import parse_score
...
def collect(conn, pass_no, manifest, projects_dir=None, session_id="unknown", since_seconds=None):
    ...
    # Incremental scrape: this scan is stateless and otherwise re-reads EVERY historical
    # transcript each call (O(tags x files x size), grows every batch). When since_seconds is
    # set, only read transcripts modified within that window — the just-finished batch's
    # cells. Safe because we collect every round, so older successes are already in the DB and
    # the manifest only holds still-missing tags (whose scores, if any, are in fresh files).
