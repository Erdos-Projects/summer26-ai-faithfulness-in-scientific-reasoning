"""SQLite layer for the SciVer verification eval.

`item` is byte-identical to `rubric_scoring.db` (same columns, same item_ids) so
the two databases join on `item_id`. `run`/`prediction` are the verdict-aware
analogs of the rubric `pass`/`annotation` tables, keeping the same per-call token
provenance columns.
"""
import sqlite3
...
CREATE TABLE IF NOT EXISTS item(
  ...
  vtype TEXT, image_path TEXT, claim TEXT, label INTEGER);
